...
//  세션에서 전문가의 expertIdx 뽑아오기
    ...
    @ResponseBody
    public int getLoggedInExpertIdx(HttpServletRequest request) {
        HttpSession session = request.getSession(false);
        int getExpertIdx = (int) session.getAttribute(SessionConst.Login_Member);
        log.info("getExpertIdx: {}", getExpertIdx);
        return getExpertIdx;
    }
...
//  전문가가 수정하기 이전의 카테고리가 무엇인지 출력해주기
    ...
    @ResponseBody
    public ResponseEntity<Category> getExpertCategory(HttpServletRequest request) {
        int expertIdx = getLoggedInExpertIdx(request);
